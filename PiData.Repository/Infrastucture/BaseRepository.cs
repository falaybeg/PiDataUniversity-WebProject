...
namespace PiDataApp.Repository.Infrastucture
{
    public class BaseRepository<T> : IBaseRespository<T> where T : class
    {
        ...
        public IEnumerable<T> PageRecord(Expression<Func<T, bool>> whereCondition, Expression<Func<T, string>> orderBy, int pageNo, int pageSize)
        {
            var result = dbSet.Where(whereCondition).OrderBy(orderBy).Skip((pageNo - 1) * pageSize).Take(pageSize);
            return result;
        }
